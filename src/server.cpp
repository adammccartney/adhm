...
// until we implement a hashable table in the next chapter.
...
static bool try_flush_buffer(Conn *conn) {
  ...
  if (conn->wbuf_sent == conn->wbuf_size) {
    // response was fully snt, change state back
    conn->state = STATE_REQ;
    conn->wbuf_sent = 0;
    conn->wbuf_size = 0;
    return false;
...
static bool try_one_request(Conn *conn) {
  ...
  if (4 + len > conn->rbuf_size) {
    // not enough data in the buffer. Will retry in the next iterration
    return false;
  }
  ...
  uint32_t rescode = 0;
  uint32_t wlen = 0;
  int32_t err = do_request(&conn->rbuf[4], len, &rescode, &conn->wbuf[4 + 4],
                         &wlen);
  if (err) {
    conn->state = STATE_END;
    return false;
  ...
  // note: frequent memove is inefficient
  // not: need for better handling for production code
...
int main(void) {
  ...
  addr.sin_addr.s_addr = ntohl(0); // wilcard 0.0.0.0
